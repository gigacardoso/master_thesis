...
public class Approach1CreateDataNormalized {
	...
	public Approach1CreateDataNormalized(String output, int steps, ArrayList<Double> mins, ArrayList<Double> maxs) {
	...
	public void createDataSVC(int stp) throws IOException{
		...
		outSVC6.write(att6+'\n');
		outSVC7.write(att7+'\n');
		CreateDataNormalized t = new CreateDataNormalized();
		...
		while ((line1 = inSVC.readLine()) != null) {
			...
			if(last == Integer.parseInt(splited[0])){
				...
				for(int i=0 ; i < steps-1; i++){
					...
					att2 += t.normalize(3,split[2]) + ",";
					att5 += t.normalize(4,split[5]) + ",";
					att6 += t.normalize(5,split[6])+ ",";
					att7 += t.normalize(6,split[7]) + ",";
				}
		...
		CreateDataNormalized c = new CreateDataNormalized();
	...
	public void createDataVitals(int stp) throws IOException{
		...
		while ((line1 = inVitals.readLine()) != null) {
			...
			if(last == Integer.parseInt(splited[0])){
				...
				for(int i=0 ; i < steps-1; i++){
					split = lines1.get(((lines1.size()-steps)+i)).split(",",-1);
					att2 += t.normalize(7,split[2]) + ",";
					att3 += t.normalize(8,split[3]) + ",";
					att6 += t.normalize(9,split[6]) + ",";
					att7 += t.normalize(10,split[7]) + ",";
					att8 += t.normalize(11,split[8]) + ",";
					att9 += t.normalize(12,split[9]) + ",";
				}
				split = lines1.get((lines1.size()-1)).split(",",-1);
				att2 += t.normalize(7,split[2]);
				att3 += t.normalize(8,split[3]);
