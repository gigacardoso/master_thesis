...
d <- read.csv(file=paste(c("C:\\ALSHMM\\multidata\\",steps,"\\nomissing\\approach2_",exam,"_",steps,".csv"),collapse=""),head=TRUE,sep=",", stringsAsFactors=FALSE)
...
vals <- c("Male","Female","B0","B1","B2","B3","B4","B5","missing","$")

rows <- nrow(d)
fold <- floor(rows/10)

folds <- c(1,fold,fold*2,fold*3,fold*4,fold*5,fold*6,fold*7,fold*8,fold*9)
fileConn<-file(paste(c("C:\\PROACT_2013_08_27_ALL_FORMS\\hmm\\multidata\\",exam,"_Predictions.csv"),collapse=""))
...
getPossibleValues <- function(exam){
	if(exam == "Demo1"){
		vals <- c("Female","Male","missing")
	}else{
		vals <- c("B0","B1","B2","B3","B4","B5","missing")
	}
	#print(vals)
	return(vals)
}
